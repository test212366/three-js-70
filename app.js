...
html2canvas(document.querySelector('#caputre')).then(canvas => {
	...
	datum.forEach((imageDat, i) => {
		let cloned = canvas.cloneNode()
		cloned.style.transition = `all 1.5s ease-out ${i/36 + 1.5}s`
		cloned.getContext('2d').putImageData(imageDat, 0,0)
		document.body.appendChild(cloned)
		setTimeout(() => {
			let angle = (Math.random() - 0.5 ) * 2 * Math.PI
			let rotateAngle = 15 * (Math.random() - 0.5)
			cloned.style.transfom = `rotate(${rotateAngle}deg) transtale(${60*Math.cos(angle)}px, ${60*Math.sin(angle)}px)`
